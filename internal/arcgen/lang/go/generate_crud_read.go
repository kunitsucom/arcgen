...
func generateREADContent(astFile *ast.File, arcSrcSet *ARCSourceSet) {
	for _, arcSrc := range arcSrcSet.ARCSourceSlice {
		...
		for _, hasOneTag := range tableInfo.HasOneTags {
			// const Find{StructName}By{FieldName}Query = `SELECT {column_name1}, {column_name2} FROM {table_name} WHERE {pk1} = ? [AND ...]`
			//
			//	// Find{StructName}
			//	func (q *query) Find{StructName}ByColumn1[AndColumn2](ctx context.Context, queryer sqlContext, {ColumnName} {ColumnType} [, {Column2Name} {Column2Type}]) ({Struct}Slice, error) {
			//		rows, err := queryer.QueryContext(ctx, Find{StructName}Query, {ColumnName}, {Column2Name})
			// 		if err != nil {
